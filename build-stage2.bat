...
set RUST_VERSION=1.93.0-2
if exist dist-rust/rust-%RUST_VERSION%-x86_64.pkg.tar.zst (
  echo "Building rust by cross use exist"
  cp -arf dist-rust/rust-%RUST_VERSION%-x86_64-cross.pkg.tar.zst ports/rust/rust-%RUST_VERSION%-x86_64.pkg.tar.zst
...
cp -arf ports/rust/rust-%RUST_VERSION%-x86_64.pkg.tar.zst dist-rust/rust-%RUST_VERSION%-x86_64-cross.pkg.tar.zst
...
if exist dist-rust/rust-%RUST_VERSION%-x86_64.pkg.tar.zst (
  ...
  cp -arf dist-rust/rust-%RUST_VERSION%-x86_64.pkg.tar.zst ports/rust/rust-%RUST_VERSION%-x86_64.pkg.tar.zst
) else (
...
set CHERE_INVOKING=1
pushd dist-final
bash --login -c "pacman -U --noconfirm --overwrite \* ./rust-%RUST_VERSION%-x86_64.pkg.tar.zst ./gcc-libs-15.2.0-2-x86_64.pkg.tar.zst ./gcc-15.2.0-2-x86_64.pkg.tar.zst"
popd
